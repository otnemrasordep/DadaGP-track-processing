    ...
    for i,token in enumerate(body):
        ...
        if token=="new_measure":
            ...
            if(t[0]=="measure"):
                ...
                if "trackbeats" not in this_measure.keys():
                    this_measure["trackbeats"] ={}        
...
"""
    if len(sys.argv)>=4 and sys.argv[1] in ["encode", "decode"]:
    ...
    elif len(sys.argv)>=1 and sys.argv[1] in ["decode_folder"]:
        
        path = str(input("Enter folder path: "))
        destination_folder = os.path.join(path, "gps")
        ...
        for file in os.listdir(path):
            if ".txt" in file:
                ...
                out_file = file.split(".txt")[0]
                dadagp_decode(os.path.join(path,file), os.path.join(destination_folder, out_file + ".gp5"))
